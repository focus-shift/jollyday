...
class JacksonFixedWeekdayInMonthTest {
  ...
  void ensureFieldsAreSetAndMappedCorrectly() {
    ...
    fixedWeekdayInMonth.setMovingCondition(Collections.singletonList(movingCondition));
    ...
    assertThat(jacksonFixedWeekdayInMonth.validFrom()).hasValue(Year.of(2012));
    assertThat(jacksonFixedWeekdayInMonth.validTo()).hasValue(Year.of(2022));
  }
  ...
  void ensureToReturnNullOrDefaultValuesOnNotSetValues() {
    ...
    fixedWeekdayInMonth.setMonth(Month.JUNE);
    fixedWeekdayInMonth.setWhich(Which.LAST);
    fixedWeekdayInMonth.setMovingCondition(Collections.emptyList());

    final JacksonFixedWeekdayInMonth jacksonFixedWeekdayInMonth = new JacksonFixedWeekdayInMonth(fixedWeekdayInMonth);
    assertThat(jacksonFixedWeekdayInMonth.weekday()).isEqualTo(DayOfWeek.FRIDAY);
    assertThat(jacksonFixedWeekdayInMonth.month()).isEqualTo(java.time.Month.JUNE);
    assertThat(jacksonFixedWeekdayInMonth.which()).isEqualTo(Occurrence.LAST);
    assertThat(jacksonFixedWeekdayInMonth.cycle()).isEqualTo(YearCycle.EVERY_YEAR);
    assertThat(jacksonFixedWeekdayInMonth.descriptionPropertiesKey()).isNull();
    assertThat(jacksonFixedWeekdayInMonth.holidayType()).isEqualTo(de.focus_shift.jollyday.core.HolidayType.PUBLIC_HOLIDAY);
    assertThat(jacksonFixedWeekdayInMonth.validFrom()).isEmpty();
    assertThat(jacksonFixedWeekdayInMonth.validTo()).isEmpty();
  }
}
