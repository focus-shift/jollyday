...
class FixedParserTest {
  ...
  void testCycle2YearsReverseInvalidRepeat() {
    ...
    final List<Holiday> holidays = sut.parse(Year.of(2009), new JacksonHolidays(config));
    assertThat(holidays).isEmpty();
  }
  ...
  void testCycle3Years() {
    ...
    final Holidays config = createHolidays(fixed);
    final List<Holiday> holidays = sut.parse(Year.of(2013), new JacksonHolidays(config));
  ...
  void testCycle3YearsReverse() {
    ...
    fixed.setEvery(HolidayCycleType.THREE_YEARS);
    final Holidays config = createHolidays(fixed);
    final List<Holiday> holidays = sut.parse(Year.of(2007), new JacksonHolidays(config));
    assertThat(holidays).hasSize(1);
  }
  ...
  void testCycle5YearsInvalidBeforeStart() {
    ...
    final List<Holiday> holidays = sut.parse(Year.of(2005), new JacksonHolidays(config));
    assertThat(holidays).isEmpty();
  }
  ...
  void testCycle5YearsReverseInvalidAfterEnd() {
    ...
    final List<Holiday> holidays = sut.parse(Year.of(2015), new JacksonHolidays(config));
    assertThat(holidays).isEmpty();
  }
