...
public class FindWeekDayRelativeToDate
    implements Function<FixedWeekdayRelativeToFixedHolidayConfiguration, LocalDate> {
  ...
  @Override
  public @NonNull LocalDate apply(
      @NonNull final FixedWeekdayRelativeToFixedHolidayConfiguration fixedWeekdayRelativeToFixed) {
    ...
    result =
        fixedWeekdayRelativeToFixed.when() == Relation.AFTER
            ? result.plusDays(days)
            : result.minusDays(days);
    return result;
  }
   ...
   */
  private @NonNull LocalDate moveDateToFirstOccurrenceOfWeekday(
      @NonNull final FixedWeekdayRelativeToFixedHolidayConfiguration fixedWeekdayRelativeToFixed,
   ...
   */
  private int determineNumberOfDays(
      @NonNull final FixedWeekdayRelativeToFixedHolidayConfiguration fixedWeekdayRelativeToFixed) {
  ...
  /**
   * Returns the closest day-of-week adjuster, which adjusts the date to the first occurrence of the
   * specified day-of-week closest to the date being adjusted unless it is already on that day in
   * which case the same object is returned.
   *
   * <p>The ISO calendar system behaves as follows:<br>
   * The input 2011-01-15 (a Saturday) for parameter (MONDAY) will return 2011-01-17 (two days
   * later).<br>
   * The input 2011-01-15 (a Saturday) for parameter (WEDNESDAY) will return 2011-01-12 (three days
   * earlier).<br>
   * The input 2011-01-15 (a Saturday) for parameter (SATURDAY) will return 2011-01-15 (same as
   * input).
   *
