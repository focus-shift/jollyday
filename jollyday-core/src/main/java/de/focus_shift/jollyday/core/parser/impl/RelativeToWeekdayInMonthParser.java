 ...
 *
 * <p>Resolves the holiday date for a given relative-to-weekday-in-month configuration.
 *
 * <p>This method works as follows:
 *
 * <ul>
 *   <li>Finds the base date in the month using the weekdayInMonth configuration.
 *   <li>Calculates the difference in days to the target weekday, considering the direction (BEFORE
 *       or AFTER).
 *   <li>Returns a Holiday for the resulting date.
 * </ul>
 */
    ...
    return holidays.relativeToWeekdayInMonth().stream()
        ...
        .map(
            relativeToWeekdayInMonth ->
                resolveHolidayForRelativeToWeekdayInMonth(year, relativeToWeekdayInMonth))
        .toList();
  }

  private @NonNull Holiday resolveHolidayForRelativeToWeekdayInMonth(
      @NonNull final Year year, @NonNull final RelativeToWeekdayInMonthHolidayConfiguration rwm) {
