...
/**
 * Abstract base class for all holiday manager implementations. Upon call of getInstance method the
 * implementing class will be read from the jollyday.properties file and instantiated.
 */
public abstract class HolidayManager {
  ...
  /**
   * Signifies if caching of manager instances is enabled. If not every call to getInstance will
   * return a newly instantiated and initialized manager.
   */
  ...
  private static final ConfigurationProviderManager CONFIGURATION_MANAGER_PROVIDER =
      new ConfigurationProviderManager();

  /** Manager for providing configuration data sources which return the holiday data. */
  private static final ConfigurationServiceManager configurationServiceManager =
      new ConfigurationServiceManager(
          new LazyServiceLoaderCache<>(HolidayCalendarConfigurationService.class));
  ...
  /**
   * Creates a HolidayManager instance for the default locale country using the configured
  ...
  /**
   * Creates a new {@code HolidayManager} instance for the country and puts it to the manager cache.
   *
  ...
  private static @NonNull HolidayManager createManager(@NonNull final ManagerParameter parameter) {
    LOG.debug(
        "Creating HolidayManager for calendar '{}'. Caching enabled: {}",
        parameter,
    ...
    final String configurationServiceImplClassName =
        readConfigurationServiceImplClassName(parameter);
    final HolidayManagerValueHandler holidayManagerValueHandler =
        new HolidayManagerValueHandler(
            parameter,
            managerImplClassName,
            configurationServiceImplClassName,
   ...
   */
  private static @NonNull String readManagerImplClassName(
      @NonNull final ManagerParameter parameter) {
   ...
   * @return the manager implementation class name
   */
  private static @NonNull String readConfigurationServiceImplClassName(
      @NonNull final ManagerParameter parameter) {
    final String className = parameter.getConfigurationServiceImplClassName();
    if (className == null) {
      throw new IllegalStateException(
          "Missing configuration '"
              + ManagerParameter.CONFIGURATION_SERVICE_IMPL
              + "'. Cannot create configuration service.");
    }
    return className;
  }

  /**
   * If true, instantiated managers will be cached. If false every call to getInstance will create
   * new manager. True by default.
   *
  ...
  /**
   * Returns true or false if the requested calendar date is a holiday in the state or the optional
   * given region and below
   *
  ...
  /**
   * Returns true or false if the requested calendar date is a holiday in the state or based on the
   * given {@link HolidayType} and the optional given region and below
   *
   * @param calendar {@link java.util.Calendar} to check.
  ...
  public boolean isHoliday(
      ...
      @Nullable final HolidayType holidayType,
      @NonNull final String... args) {
    return isHoliday(new CalendarToLocalDate().apply(calendar), holidayType, args);
  }

  /**
   * Returns true or false if the requested date is a holiday in the state or the optional given
   * region and below
   *
   ...
   * {@link HolidayType} and the optional given region and below
   *
   * @param localDate The potential holiday.
   * @param holidayType a {@link HolidayType} to be considered
   * @param args Hierarchy to request the holidays for. i.e. args = {'ny'} -&gt; New York holidays
  ...
  public void setConfigurationService(
      @NonNull final HolidayCalendarConfigurationService configurationService) {
    this.configurationService = configurationService;
  }

  /**
   * Returns the {@link HolidayCalendarConfigurationService} to be used to retrieve holiday data.
   *
   ...
   *
   * @param year i.e. 2010
   * @param args i.e. args = {'ny'}. returns US/New York holidays. No args means holidays common to
   *     whole country
   * @return a set of holidays for the requested year
   */
  public abstract @NonNull Set<Holiday> getHolidays(
      @NonNull final Year year, @NonNull final String... args);

  /**
   * Returns the holidays for the requested year, the given {@link HolidayType} and the hierarchy
   * structure
   *
   * @param year i.e. 2010
   * @param holidayType a {@link HolidayType} to be considered
  ...
  /**
   * Returns the holidays for the requested interval, the given {@link HolidayType} and hierarchy
   * structure.
   *
   * @param startDateInclusive the start date of the interval in which holidays lie, inclusive
   * @param endDateInclusive the end date of the interval in which holidays lie, inclusive
