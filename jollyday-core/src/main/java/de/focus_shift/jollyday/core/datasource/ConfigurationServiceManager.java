...
/**
 * This manager is responsible for instantiating the provided implementation of the {@link
 * HolidayCalendarConfigurationService} which is used to access the holiday data.
 */
public class ConfigurationServiceManager {

  private final LazyServiceLoaderCache<HolidayCalendarConfigurationService>
      configurationServiceCache;

  public ConfigurationServiceManager(
      @NonNull final LazyServiceLoaderCache<HolidayCalendarConfigurationService>
              configurationServiceCache) {
  ...
  public @NonNull HolidayCalendarConfigurationService getConfigurationService(
      @NonNull final String configurationServiceImplClassName) {
    return instantiateDataSource(configurationServiceImplClassName);
  }

  private @NonNull HolidayCalendarConfigurationService instantiateDataSource(
      @NonNull final String configurationServiceImplClassName) {

    final List<HolidayCalendarConfigurationService> services =
        configurationServiceCache.getServices();
      ...
      return services.stream()
          .filter(
              configurationService ->
                  configurationServiceImplClassName.equals(
