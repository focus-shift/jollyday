...
/**
 * Manager implementation for reading data from the configuration datasource. It uses a list a
 * parsers for parsing the different type of XML nodes.
 */
public class DefaultHolidayManager extends HolidayManager {
  ...
  /**
   * Caches the instance based country specific holidays so that e.g. the created HolidayManager via
   * {@code HolidayManager.getInstance(create("de"))} only contains the german holidays.
   *
   * <p>This is also the reason this cache is not static. If it was static all holidays over all
   * holiday manager instances would be cached, but only the german holidays are important for the
   * german holiday manager, so only save them.
   */
   ...
   * {@inheritDoc}
   *
   * <p>Initializes the DefaultHolidayManager by loading the holidays XML file as resource from the
   * classpath. When the XML file is found it will be unmarshalled with JAXB to some Java classes.
   */
  @Override
  public void doInit() {
    holidayCalendarConfiguration =
        getConfigurationService().getHolidayCalendarConfiguration(getManagerParameter());
   ...
   * {@inheritDoc}
   *
   * <p>Calls <code>Set&lt;LocalDate&gt; getHolidays(Year year, Configuration c, String... args)
   * </code> with the configuration from initialization.
   */
  @Override
  public @NonNull Set<Holiday> getHolidays(
      @NonNull final Year year, @NonNull final String... args) {
    ...
    final Cache.ValueHandler<Set<Holiday>> holidayValueHandler =
  ...
  public @NonNull Set<Holiday> getHolidays(
      ...
      @NonNull final HolidayType holidayType,
   ...
   * {@inheritDoc}
   *
   * <p>Calls <code>getHolidays(year, args)</code> for each year within the interval and returns a
   * list of holidays which are then contained in the interval.
   */
  @Override
  public @NonNull Set<Holiday> getHolidays(
      @NonNull final LocalDate startDateInclusive,
      @NonNull final LocalDate endDateInclusive,
      @NonNull final String... args) {
    ...
    return rangeClosed(startDateInclusive.getYear(), endDateInclusive.getYear())
        ...
        .filter(
            holiday ->
                !startDateInclusive.isAfter(holiday.getDate())
                    && !endDateInclusive.isBefore(holiday.getDate()))
   ...
   * {@inheritDoc}
   *
   * <p>Returns the configurations hierarchy.<br>
  ...
  private void getHolidays(
      @NonNull final Year year,
      @NonNull final HolidayCalendarConfiguration configuration,
      @NonNull final Set<Holiday> holidaySet,
      @NonNull final String... args) {
    ...
    if (args != null && args.length > 0) {
      ...
      configuration
          ...
          .forEach(
              config -> getHolidays(year, config, holidaySet, copyOfRange(args, 1, args.length)));
    }
  }
  ...
  private void parseHolidays(
      @NonNull final Year year,
      @NonNull final Set<Holiday> holidays,
      @NonNull final HolidayConfigurations config) {
  ...
  /**
   * Creates a list of parsers by reading the configuration and trying to find an <code>
   * HolidayParser</code> implementation for by XML class type.
   *
   * @param config the holiday configuration
   * @return A list of parsers to for this configuration.
   */
  private @NonNull Collection<HolidayParser> getParsers(
      @NonNull final HolidayConfigurations config) {
  ...
  private @NonNull HolidayParser instantiateParser(@NonNull final String className) {
    ...
    final Cache.ValueHandler<HolidayParser> parserValueHandler =
        new Cache.ValueHandler<>() {
          @Override
          public @NonNull String getKey() {
            return className;
          ...
          public @NonNull HolidayParser createValue() {
            final String parserClassName =
                getManagerParameter().getProperty(PARSER_IMPL_PREFIX + className);
            ...
            throw new IllegalStateException(
                "Cannot create parsers. No parser implementation defined for class "
                    + className
                    + " in properties with key "
                    + PARSER_IMPL_PREFIX
                    + className);
          }
        };
   ...
   * @param level an int.
   */
  protected static void logHierarchy(
      @NonNull final HolidayCalendarConfiguration configuration, final int level) {
    if (LOG.isDebugEnabled()) {
      ...
      configuration.subConfigurations().forEach(config -> logHierarchy(config, level + 1));
    }
  }
  ...
  protected static void validateConfigurationHierarchy(
    ...
    configuration
        .subConfigurations()
        .forEach(DefaultHolidayManager::validateConfigurationHierarchy);
  }
   ...
   */
  private static @NonNull CalendarHierarchy createConfigurationHierarchy(
      @NonNull final HolidayCalendarConfiguration configuration,
      @Nullable final CalendarHierarchy calendarHierarchy) {
    final CalendarHierarchy hierarchy =
        new CalendarHierarchy(calendarHierarchy, configuration.hierarchy());
    hierarchy.setFallbackDescription(configuration.description());
    configuration
        .subConfigurations()
        .forEach(
            subConfiguration -> {
              final CalendarHierarchy subHierarchy =
                  createConfigurationHierarchy(subConfiguration, hierarchy);
