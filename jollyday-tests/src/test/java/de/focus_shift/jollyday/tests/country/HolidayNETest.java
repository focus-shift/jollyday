...
import static de.focus_shift.jollyday.core.HolidayCalendar.NIGER;
...
class HolidayNETest {
  ...
  void ensuresHolidays() {
    assertFor(NIGER)
      ...
      .hasFixedHoliday("REFOUNDATION_DAY", MARCH, 26).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("CONCORD_DAY", APRIL, 24).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("LABOUR_DAY", MAY, 1).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("CNSP_COUP_ANNIVERSARY", JULY, 26).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("INDEPENDENCE_DAY", AUGUST, 3).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("REPUBLIC_DAY", DECEMBER, 18).validBetween(YEAR_FROM, YEAR_TO).and()
