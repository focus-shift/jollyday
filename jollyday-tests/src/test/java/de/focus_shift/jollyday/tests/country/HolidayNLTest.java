...
class HolidayNLTest {
  ...
  void ensuresHolidays() {
    assertFor(NETHERLANDS)
        ...
        .and()
        .hasFixedHoliday("FIRST_CHRISTMAS_DAY", DECEMBER, 25)
        .and()
  ...
  @Property
  void ensuresThatAllLiberationIsConfiguredEveryFiveYearsSince1945Until1989(
      @ForAll @YearRange(min = 1945, max = 1989) Year year) {
    ...
    if (year.getValue() % 5 == 0) {
      assertThat(holidays)
          .isNotEmpty()
          .contains(
              new Holiday(LocalDate.of(year.getValue(), MAY, 5), "LIBERATION", PUBLIC_HOLIDAY));
    }
  }
}
