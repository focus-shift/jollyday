...
public abstract class AbstractCountryTestBase {
  ...
  protected void validateCalendarData(
      final String countryCode, Year year, boolean assertAllHolidaysChecked) {
    final HolidayManager dataManager =
        HolidayManager.getInstance(ManagerParameters.create(countryCode));
    final HolidayManager testManager =
        HolidayManager.getInstance(ManagerParameters.create("test_" + countryCode + "_" + year));
  ...
  protected void compareHierarchies(CalendarHierarchy expected, CalendarHierarchy found) {
    ...
    assertThat(found.getChildren())
        .withFailMessage(
            "Number of %s children should be %s but are %s",
            found.getDescription(), expected.getChildren().size(), found.getChildren().size())
  ...
  protected void compareData(
      HolidayManager expected, HolidayManager found, Year year, boolean assertAllHolidaysChecked) {
    compareDates(
        expected,
        found,
        expected.getCalendarHierarchy(),
        List.of(),
        year,
        assertAllHolidaysChecked);
  }

  private void compareDates(
      final HolidayManager expected,
      final HolidayManager found,
      final CalendarHierarchy expectedHierarchy,
      ...
      if (!CalendarUtil.contains(foundHolidays, expectedHoliday.getDate())) {
        fail(
            "Could not find "
                + expectedHoliday
                + " in "
                + expectedHierarchy.getDescription()
                + " -  "
                + foundHolidays);
      }
    }
    ...
    if (assertAllHolidaysChecked) {
      assertThat(expectedHolidays.stream().sorted().toList())
          .describedAs(expectedHierarchy.getDescription())
    ...
    for (String id : expectedHierarchy.getChildren().keySet()) {
      ...
      compareDates(
          expected,
          found,
          expectedHierarchy.getChildren().get(id),
          newArgs,
          year,
          assertAllHolidaysChecked);
    }
