...
import static de.focus_shift.jollyday.core.HolidayCalendar.TOGO;
...
class HolidayTGTest {
  ...
  void ensuresHolidays() {
    ...
    assertFor(TOGO)
      ...
      .hasFixedHoliday("LIBERATION_DAY", JANUARY, 13).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("INDEPENDENCE_DAY", APRIL, 27).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("LABOUR_DAY", MAY, 1).validBetween(YEAR_FROM, YEAR_TO).and()
      .hasFixedHoliday("MARTYRS_DAY", JUNE, 21).validBetween(YEAR_FROM, YEAR_TO).and()
